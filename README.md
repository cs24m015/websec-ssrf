...
You can test the SSRF by opening the Burp Suite browser and navigate to `http://localhost:5050`. Afterwards you need to send the request to the intrudor, add a payload type `Numbers` at the url IP address and configure the range of the last octet from 100 up to 110.
...
There is no validation or sanitization of the URL input. An attacker can send a request to the fetch endpoint with a URL that points to internal services, such as http://172.20.0.110:5050/
